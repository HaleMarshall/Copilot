...
// Choose a readable label colour (near-black or white) for text drawn directly ON a fill colour,
// from the fill's perceived luminance — so inside bar/slice labels stay legible on both light
// (cream/tan) and dark (blue/navy/ink) backgrounds, with no white halo.
function readableOn(bg, fallback) {
  fallback = fallback || '#0E0E0E';
  if (typeof bg !== 'string') return fallback;
  let r, g, b;
  ...
  // perceived luminance (ITU-R BT.601); >150 ⇒ light bg ⇒ dark text, else white.
  return (0.299 * r + 0.587 * g + 0.114 * b) > 150 ? '#0E0E0E' : '#FFFFFF';
}
  ...
  afterDatasetsDraw(chart, _a, pluginOpts) {
    ...
    chart.data.datasets.forEach((ds, i) => {
      ...
      meta.data.forEach((el, idx) => {
        ...
        const elBg = Array.isArray(bgArr) ? bgArr[idx] : bgArr;
        // inside: adapt to the bar's own fill; outside: dark text on the light panel.
        ctx.fillStyle = mode === 'outside' ? (pick(opts.outsideColor) || '#0E0E0E') : readableOn(elBg, pick(opts.color));
...
function drawPieLabels(chart, ctx, ds, meta, opts) {
  ...
  pts.forEach(it => {
    ...
    if (it.frac >= 0.06 && arcLen >= fs + 8) {
      ...
      ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
      const sliceBg = Array.isArray(ds.backgroundColor) ? ds.backgroundColor[it.idx] : ds.backgroundColor;
      ctx.fillStyle = readableOn(sliceBg, opts.color); ctx.fillText(pct(it.v), x, y);
    } else {
      outside.push(it);
    }
  });
    ...
    data: { labels: [`You (of ${state.profile.targetPct}% target)`, 'Peers', 'Platform average', 'Moonfare Model target'],
      datasets: [
        { label:'Filled',    data:[fillYou, fillPeer, fillPlat, fillModel],
          ...
          backgroundColor: [COL.blue, COL.peer, COL.platform, COL.goldman], borderRadius: 4, barThickness: 'flex', maxBarThickness: 46, categoryPercentage: 0.82, barPercentage: 0.9 },
        { label:'Remaining', data:[remaining(fillYou), remaining(fillPeer), remaining(fillPlat), remaining(fillModel)],
          backgroundColor: '#EEE8DC', borderRadius: 4, barThickness: 'flex', maxBarThickness: 46, categoryPercentage: 0.82, barPercentage: 0.9 }
      ]
    },
    options: {
      indexAxis: 'y', responsive: true, maintainAspectRatio: false,
